...
// 	id := 0
// 	for {
// 		idToSession[id] = Session{time.Now().String(), time.Now().String()}
// 		for _, s := range spots {
// 			fmt.Print(s + ": ")
...
// 			if len(txt) > 0 {
// 				matches := r.MatchString(txt)
// 				if matches {
// 					fg := parseString(txt)
// 					if s == "midrange_left_corner" {
// 						midrangeLeftCorner[id] = fg
// 					}
// 				} else {
...
// 		}
// 		fmt.Println("midrangeLeftCorner = ", midrangeLeftCorner)
// 		id++
// 	}
// }

func main() {
	fmt.Println("terminal implementation")
}
